...
// a thin rendered DOM (82KB shell). The 10KB absolute API-size threshold
// should fire.
...
// small API response (5KB, below the 10KB threshold) does NOT trigger
// detection. This prevents false positives on pages that make modest API
// calls alongside a full DOM render.
func TestDetectContentCandidates_NoFireForSmallAPI(t *testing.T) {
	m := browser.NewNetworkMonitor()

	// Small config API: 5KB JSON (below 10KB threshold).
...
// API responses are all captured when the 10KB threshold is exceeded.
func TestDetectContentCandidates_MultipleCandidates(t *testing.T) {
	...
	// Total API = 80KB (well above 10KB threshold).
	signals := &browser.DOMSignals{IsSPA: true}
	candidates := detectContentCandidates(context.Background(), m, signals)
...
// TestDetectContentCandidates_LargeAPI verifies that a single API response
// above the 10KB threshold triggers detection.
func TestDetectContentCandidates_LargeAPI(t *testing.T) {
	...
	if len(candidates) != 1 {
		t.Fatalf("expected 1 candidate for large API response, got %d", len(candidates))
	}
}
...
func TestDetectContentCandidates_NilSignals(t *testing.T) {
	...
	candidates := detectContentCandidates(context.Background(), m, nil)
	if candidates != nil {
