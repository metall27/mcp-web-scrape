...
func (t *ScrapeJSTool) Execute(ctx context.Context, args map[string]interface{}) (map[string]interface{}, error) {
	...
	// Build result in MCP format. When auth failure / blocking / fallback is
	// detected, prepend a diagnostic banner so any MCP client — even one that
	// discards the non-standard _metadata — delivers the diagnosis to the LLM
	// in content[0].text (issue #87).
	resultHTML := result.HTML
	if banner := buildDiagnosticBanner(result); banner != "" {
		resultHTML = banner + resultHTML
		t.logger.Info().
			Str("final_url", RedactURL(result.FinalURL)).
			Bool("has_login_form", result.DOMSignals != nil && result.DOMSignals.HasLoginForm).
			Int("auth_failures", authFailureCount(result)).
			Msg("Diagnostic banner prepended to content (#87)")
	}
